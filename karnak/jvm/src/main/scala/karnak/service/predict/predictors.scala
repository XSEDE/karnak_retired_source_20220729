...
object UnsubmittedPredictor extends PredictorUtils with Logging {
  def apply(name: String): UnsubmittedPredictor = {
    ...
    new UnsubmittedPredictor(name,dropTime(KarnakDatabase.getUnsubmittedSchema(name)))
  }

  @throws(classOf[KarnakException])
  def getPrediction(query: UnsubmittedStartTimeQuery): UnsubmittedStartTimePrediction = {
  ...
  private var predictors = Map[String,UnsubmittedPredictor]()
	
  @throws(classOf[KarnakException])
  private def getPredictor(system: String): UnsubmittedPredictor = {
    if (!predictors.contains(system)) {
      predictors = predictors + (system -> UnsubmittedPredictor.apply(system))
    }
    predictors(system)
  }
...
class UnsubmittedPredictor(name: String, schema: ExperienceSchema) extends WaitTimePredictor(name, schema) {
...
object SubmittedPredictor extends PredictorUtils with Logging {
  def apply(name: String): SubmittedPredictor = {
    ...
    new SubmittedPredictor(name,dropTime(KarnakDatabase.getSubmittedSchema(name)))
  }

  @throws(classOf[KarnakException])
  def getPrediction(query: SubmittedStartTimeQuery): SubmittedStartTimePrediction = {
    logger.info("predict")
  ...
  private var predictors = Map[String,SubmittedPredictor]()
	
  @throws(classOf[KarnakException])
  private def getPredictor(system: String): SubmittedPredictor = {
    if (!predictors.contains(system)) {
      predictors = predictors + (system ->SubmittedPredictor.apply(system))
    }
    predictors(system)
  }
...
class SubmittedPredictor(name: String, schema: ExperienceSchema) extends WaitTimePredictor(name, schema) {
...
abstract class WaitTimePredictor(name: String, schema: ExperienceSchema) 
  ...
  private var lastUpdate = new Date(0)

  private def updatePredictorsIfNecessary(): Unit = {
    val now = new Date()
