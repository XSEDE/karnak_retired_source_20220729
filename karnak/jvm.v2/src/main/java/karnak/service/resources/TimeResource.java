...
public class TimeResource {
    ...
    public String getSystemsHtml() {
        ...
        text = text + "<ul>";
        for (String system : systems) {
            text = text + "<li> <a href='system/" + system + "/job/waiting.html'>" + system + "</a></li>\n";
        }
        text = text + "</ul>";
        return text;
    }
    ...
    public String getPredictHtml(boolean waitPrediction,
        ...
        for (String queueSystem : queueAtSystems) {
            ...
            if (selectedSystems.contains(query.system)) {
                ...
                try {
                    UnsubmittedStartTimePrediction pred = UnsubmittedPredictor.getPrediction(query, WeightingEnum.Bagging);
                    if (waitPrediction) {
    ...
    protected String predict(PredictionRequest req) {
        ...
        for (Location location : req.locations) {
            ...
            if (req.wallTimeSecs > maxWallTime) {
                location.pred = null;
                location.errorMessage = "requested wall time must be less than " + WebService.hms(maxWallTime);
                continue;
            }
            ...
            try {
                ...
                location.pred = UnsubmittedPredictor.getPrediction(query, WeightingEnum.Bagging);
            } catch (KarnakException e) {
    ...
    /* 
     [tickets.xsede.org #42229] XUP: Queue Prediction for Comet Compute Queue Incorrect.
     
     Karnak compares the requested # of cores with maxProcessor per node instead of
     the product of processors per node (24) and the # of nodes (72).
     That means we have restricted the number of cores user requested to a very small range incorrectly.

     So we need to correct the input validation routine to check whether users requested
     number of cores greater than 1728 (= 24 x 72) cores.
     Users have requested # of cores at compute@comet.sdsc.xsede.org ranged from 1 to 12228.
     Invalid requests include 3072, 6144, 12228 cores.
 
    
     Snapshot of the system_info table at glue2 database 03/17/2016
     +--------------------------------+---------------------+------------+--------------+
     | system                         | time                | processors | procsPerNode |
     +--------------------------------+---------------------+------------+--------------+
     | blacklight.psc.xsede.org       | 2015-09-08 17:28:02 |       8192 |         2730 |
     | comet.sdsc.xsede.org           | 2016-03-17 16:42:02 |      46024 |           24 |
     | darter.nics.xsede.org          | 2016-03-17 16:44:16 |      12224 |           16 |
     | gordon.sdsc.xsede.org          | 2016-03-17 16:44:02 |      14736 |           16 |
     | lonestar4.tacc.xsede.org       | 2016-03-17 16:44:02 |      23784 |           12 |
     | maverick.tacc.xsede.org        | 2016-03-17 16:45:01 |       2740 |           20 |
     | staff.stampede.tacc.utexas.edu | 2015-06-04 16:24:22 |     103056 |           16 |
     | stampede.tacc.xsede.org        | 2016-03-17 16:40:02 |     103056 |           16 |
     | trestles.sdsc.xsede.org        | 2015-05-01 16:48:01 |       9792 |           32 |
     | wrangler.tacc.xsede.org        | 2015-05-27 21:27:17 |       4608 |           48 |
     +--------------------------------+---------------------+------------+--------------+

     1) System information what Glue2 of Karnak has
     +--------------------------------+---------------------+------------+--------------+
     | system                         | time                | processors | procsPerNode |
     +--------------------------------+---------------------+------------+--------------+
     | comet.sdsc.xsede.org           | 2016-03-17 14:04:06 |      46024 |           24 |


     2) Queue information what Glue2 of Karnak has. 
     The value -1 in the maxProcessors column means “no restriction” on the # of cores.

     +--------------------------------+-----------------------------------+---------------+-------------+-----------+---------+
     | system                         | queue                             | maxProcessors | maxWallTime | isDefault | isValid |
     +--------------------------------+-----------------------------------+---------------+-------------+-----------+---------+
     | comet.sdsc.xsede.org           | compute                           |            72 |      172800 |           |        |
     | comet.sdsc.xsede.org           | debug                             |             4 |        1800 |           |        |
     | comet.sdsc.xsede.org           | FIRO032016                        |            -1 |          -1 |           |        |
     | comet.sdsc.xsede.org           | gpu                               |             4 |      172800 |           |        |
     | comet.sdsc.xsede.org           | gpu-shared                        |             1 |      172800 |           |        |
     | comet.sdsc.xsede.org           | large-shared                      |             1 |      172800 |           |        |
     | comet.sdsc.xsede.org           | maint                             |            -1 |          -1 |           |        |
     | comet.sdsc.xsede.org           | perf_test                         |            -1 |          -1 |           |        |
     | comet.sdsc.xsede.org           | sch1                              |            -1 |          -1 |           |        |
     | comet.sdsc.xsede.org           | seagate_SSDs                      |            -1 |          -1 |           |        |
     | comet.sdsc.xsede.org           | shared                            |             1 |      172800 |           |        |
     ...
     */
    /*
     This funtion returns the aggregated number of processors that one user can request 
     for a queue. There has been a mistake in computing aggregated # of processors.
    ...
    public static int getMaxProcessors(String systemName, String queueName) {
        ...
        int procsPerNode = GlueDatabase.getSystems().get(systemName).procsPerNode;
        int maxProcessorsPerNode = GlueDatabase.getSystems().get(systemName).queues.get(queueName).maxProcessors;
