...
class Trainer extends Daemon with Logging {
  ...
  private def schedule(): Unit = {
    ...
    timer.cancel()
    timer = new Timer()
    val pathPrefixV1  = "/home/karnak/karnak/etc/predictors/v1/"; 
    val pathPrefixV2  = "/home/karnak/karnak/etc/predictors/v2/";
    val pathPrefixBackTest  = "/home/karnak/karnak/etc/predictors/backtest/";


    updateSystemNames(pathPrefixV1)
    updateSystemNames(pathPrefixV2)
    updateSystemNames(pathPrefixBackTest)

    for (system <- systems) {
      timer.schedule(new UnsubmittedTrainTask(system,4*HOUR,pathPrefixV1),0,MINUTE)
      timer.schedule(new UnsubmittedTrainTask(system,DAY   ,pathPrefixV1),0,15*MINUTE)
      timer.schedule(new UnsubmittedTrainTask(system,WEEK  ,pathPrefixV1),0,HOUR)
      timer.schedule(new UnsubmittedTrainTask(system,4*WEEK,pathPrefixV1),0,HOUR)
      timer.schedule(new UnsubmittedTrainTask(system,8*WEEK,pathPrefixV1),0,HOUR)

      timer.schedule(new SubmittedTrainTask(system,4*HOUR ,pathPrefixV1),0,MINUTE)
      timer.schedule(new SubmittedTrainTask(system,DAY    ,pathPrefixV1),0,15*MINUTE)
      timer.schedule(new SubmittedTrainTask(system,WEEK   ,pathPrefixV1),0,HOUR)
      timer.schedule(new SubmittedTrainTask(system,4*WEEK ,pathPrefixV1),0,HOUR)
      timer.schedule(new SubmittedTrainTask(system,8*WEEK ,pathPrefixV1),0,HOUR)

      //decision tree v2
      timer.schedule(new UnsubmittedTrainTaskV2(system,4*HOUR,pathPrefixV2),0,MINUTE)
      timer.schedule(new UnsubmittedTrainTaskV2(system,DAY   ,pathPrefixV2),0,15*MINUTE)
      timer.schedule(new UnsubmittedTrainTaskV2(system,WEEK  ,pathPrefixV2),0,HOUR)
      timer.schedule(new UnsubmittedTrainTaskV2(system,4*WEEK,pathPrefixV2),0,HOUR)
      timer.schedule(new UnsubmittedTrainTaskV2(system,8*WEEK,pathPrefixV2),0,HOUR)

      timer.schedule(new SubmittedTrainTaskV2(system,4*HOUR ,pathPrefixV2),0,MINUTE)
      timer.schedule(new SubmittedTrainTaskV2(system,DAY    ,pathPrefixV2),0,15*MINUTE)
      timer.schedule(new SubmittedTrainTaskV2(system,WEEK   ,pathPrefixV2),0,HOUR)
      timer.schedule(new SubmittedTrainTaskV2(system,4*WEEK ,pathPrefixV2),0,HOUR)
      timer.schedule(new SubmittedTrainTaskV2(system,8*WEEK ,pathPrefixV2),0,HOUR)
    }
  }

  private def updateSystemNames(pathPrefix: String): Unit = {
    logger.info("updating system names")
    systems = KarnakDatabase.getSystemNames().map(symbol => symbol.name).toVector
    for (system <- systems) {
      createDirectory(pathPrefix, system)
    }
  }

  private def createDirectory(pathPrefix:String, system: String): Unit = {
    val path = Paths.get(pathPrefix+system)
  ...
  class UnsubmittedTrainTask(val system: String, val history: Long, val pathPrefix: String) extends TimerTask {
    val predName = "unsubmitted-%d.json".format(history)
    val path = pathPrefix +"%s/".format(system)+predName
    ...
    private def createTrainer(name: String, schema: ExperienceSchema): TreeTrainer = {
     ...
     /* 06/07/2016 Jungha Woo
        Testing pruneFraction of 20% may simply the decision tree.
        However, its impact turned out to be insignificant so 
        pruneFraction is back to zero.
        */
  ...
  class SubmittedTrainTask(val system: String, val history: Long, val pathPrefix: String) extends TimerTask {
  ...
  class UnsubmittedTrainTaskV2(val system: String, val history: Long, val pathPrefix: String) extends TimerTask {
    val predName = "unsubmitted-%d.json".format(history)
    val path = pathPrefix +"%s/".format(system)+predName
  ...
  class SubmittedTrainTaskV2(val system: String, val history: Long, val pathPrefix: String) extends TimerTask {
    ...
    def run(): Unit = {
      ...
      val exps = dropTime(KarnakDatabase.readSubmittedObservedExperiences(system,earliestTime))
      val schema = dropTime(KarnakDatabase.getSubmittedSchema(system)).withCategories(exps)
